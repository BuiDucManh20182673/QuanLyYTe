...
public class PeopleList {
    public int insert(People peo){
        ...
        try{
            String sSQL = "insert into clinic.kbyt(name, age, gender, nationality, cccd, phone_number, address, note, vaccine) values(?,?,?,?,?,?,?,?,?)";
    ...
    public int updatePeopleByCccd(People peo) throws Exception{
        ...
        PreparedStatement pstmt = null;
        try{
        String sSQL = "update clinic.kbyt set name = ?, age = ?,  nationality = ?, cccd = ?, phone_number = ?, address = ?, note = ?, vaccine = ? where cccd = ?";
        conn = DatabaseUtils.getDBConnect();
            pstmt = conn.prepareStatement(sSQL);
            pstmt.setString(1, peo.getName());
            pstmt.setString(2, peo.getAge());
    ...
    public List<People> getAllPeople(){
        ...
        ResultSet rs = null;
        try{
            String sSQL = "select * from clinic.kbyt";
            conn = DatabaseUtils.getDBConnect();
    ...
    public int delPeopleByID(String cccd){
        ...
        try{
            String sSQL = "delete from clinic.kbyt where cccd = ?";
            conn = DatabaseUtils.getDBConnect();
    ...
    public People getPeopleByCccd(String cccd){
        ...
        People peo = new People();
        try{
            String sSQL = "select * from clinic.kbyt where cccd = ?";
            conn = DatabaseUtils.getDBConnect();
            sttm = conn.prepareStatement(sSQL);
            sttm.setString(1, cccd);
