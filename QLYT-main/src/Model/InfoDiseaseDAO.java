...
public class InfoDiseaseDAO {
    ...
    public int add(InfoDiease info){
        ...
        try{
            String sSQL = "insert into clinic.infodisease(ID, fullname, age, gender, nationality, cccd, phone, address, Fn, dose) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public int updateById(InfoDiease info){
        ...
        PreparedStatement sttm = null;
        try{
            String sSQL = "update clinic.infodisease set fullname = ?, age = ?,  gender = ?, nationality = ?, cccd = ?, phone = ?, address = ?, Fn = ?, dose = ? where ID = ?";
            conn = DatabaseUtils.getDBConnect();
            sttm = conn.prepareStatement(sSQL);
    ...
    public int delByID(String id){
        ...
        PreparedStatement sttm = null;
        try{
            String sSQL = "delete from clinic.infodisease where ID = ?";
            conn = DatabaseUtils.getDBConnect();
            sttm = conn.prepareStatement(sSQL);
            sttm.setString(1, id);
    ...
    public List<InfoDiease> getAll(){   // lay gia tri tu cau lenh sql gan vào thuoc tinh cua lop infodiese
        List<InfoDiease> ls = new ArrayList<>();
        Connection conn = null;
        Statement sttm = null;
        ResultSet rs = null;
        try{
            String sSQL = "SELECT * FROM clinic.infodisease";
            conn = DatabaseUtils.getDBConnect();
    ...
    public InfoDiease getInfoById(String id){
        ...
        InfoDiease info = new InfoDiease();
        try{
            String sSQL = "select * from clinic.infodisease where ID = ?";
            conn = DatabaseUtils.getDBConnect();
            sttm = conn.prepareStatement(sSQL);
            sttm.setString(1, id);
